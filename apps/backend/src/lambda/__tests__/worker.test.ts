...
vi.mock('../../lib/cancellation', () => ({ isUserCancelled: mocks.isUserCancelled }));
...
describe('lambda queue worker', () => {
    ...
    it('records exhausted queue failures and marks bookmark-scoped jobs as errored', async () => {
        ...
        expect(mocks.eq).toHaveBeenCalledWith('pipeline_run_id', '00000000-0000-0000-0000-000000000003');
    ...
    it('records exhausted ingest bookmark failures as untracked pipeline errors', async () => {
        ...
        const result = await processEvent('ingest', createEvent({
            queue: 'ingest',
            jobName: 'ingest',
        ...
        expect(result).toEqual({ batchItemFailures: [{ itemIdentifier: 'message-1' }] });
        expect(mocks.update).toHaveBeenCalledWith({ status: 'error' });
        expect(mocks.eq).toHaveBeenCalledWith('user_id', '00000000-0000-0000-0000-000000000001');
        expect(mocks.inFilter).toHaveBeenCalledWith('chrome_id', ['chrome-1', 'chrome-2']);
        expect(mocks.eq).toHaveBeenCalledWith('pipeline_run_id', '00000000-0000-0000-0000-000000000003');
    ...
    it('does not mutate bookmarks or notify the coordinator for stale exhausted jobs', async () => {
        const { processEvent } = await import('../worker');
        mocks.ingestProcessor.mockRejectedValueOnce(new Error('old ingest exploded'));
