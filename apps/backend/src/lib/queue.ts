...
const defaultRetryPolicyByQueue: Record<
  QueueName,
...
const queueDriver =
  ...
  (process.env.AWS_LAMBDA_FUNCTION_NAME ? "sqs" : "inline");
...
class AppQueue<T = unknown> {
  ...
  async add(jobName: string, data: T, options: QueueAddOptions = {}) {
    const jobId =
      options.jobId ??
      `${this.name}-${Date.now()}-${Math.random().toString(36).slice(2)}`;
    const retryPolicy = defaultRetryPolicyByQueue[this.name];
    const attempts = options.attempts ?? retryPolicy.attempts;
    const backoffMs =
      Number.isFinite(options.backoffMs) && Number(options.backoffMs) > 0
    ...
    if (queueDriver === "sqs") {
      ...
      if (!queueUrl) {
        throw new Error(
          `Missing ${queueUrlEnvByName[this.name]} for SQS queue ${this.name}`,
        );
      }
...
export const parseQueuedMessage = (raw: string): QueuedMessage => {
  ...
  const attempts =
    Number.isFinite(message.attempts) && Number(message.attempts) > 0
      ? Number(message.attempts)
      : retryPolicy.attempts;
  const backoffMs =
    Number.isFinite(message.backoffMs) && Number(message.backoffMs) > 0
...
export const drainTestQueuedJobs = async (maxJobs = 50) => {
  ...
  while (testQueuedJobs.length > 0 && processed.length < maxJobs) {
    ...
    if (!processor) {
      throw new Error(
        `No test processor registered for queue ${message.queue}`,
      );
    }
