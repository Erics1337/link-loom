  ...
  supabase: {
    rpc: vi.fn(async (fn: string, args: { p_user_id: string }) => {
      ...
      if (fn === "begin_user_pipeline_run") {
        ...
        return {
          data: { id: run.id, generation: nextGeneration },
          error: null,
        };
      }
      ...
      if (fn === "mark_user_cancelled") {
        ...
        for (const run of Array.from(runs.values())) {
          if (
            run.user_id === userId &&
            run.generation === (current?.job_generation ?? 0)
          ) {
    ...
    from: vi.fn((table: string) => {
      ...
      const chain = {
        ...
        maybeSingle: vi.fn(async () => ({
          data:
            table === "user_pipeline_controls"
              ? (controls.get(userId) ?? null)
              : (Array.from(runs.values()).find(
                  (run) => run.id === runId && run.user_id === userId,
                ) ?? null),
          error: selectError,
        })),
      };
...
describe("durable cancellation controls", () => {
  ...
  it("treats older queued work as cancelled after a new generation starts", async () => {
    ...
    expect(
      await isUserCancelled("user-1", staleRun.generation, staleRun.id),
    ).toBe(true);
  ...
  it("reuses lookup cache across repeated cancellation checks", async () => {
    ...
    expect(await isUserCancelled("user-1", run.generation, run.id, cache)).toBe(
      false,
    );
    expect(await isUserCancelled("user-1", run.generation, run.id, cache)).toBe(
      false,
    ...
    expect(
      fromCalls.filter(([table]) => table === "user_pipeline_controls"),
    ).toHaveLength(1);
    expect(
      fromCalls.filter(([table]) => table === "pipeline_runs"),
    ).toHaveLength(1);
  });
});
