...
describe('pipeline run SQL migrations', () => {
    it('links bookmarks to pipeline runs for run-scoped readiness checks', () => {
        expect(pipelineSql).toContain('ADD COLUMN IF NOT EXISTS pipeline_run_id UUID REFERENCES public.pipeline_runs(id)');
    ...
    it('claims clustering only after bookmarks for the current run are terminal', () => {
        const block = functionBlockFromSql(pipelineSql, 'claim_user_pipeline_clustering');
    ...
    it('stages terminal bookmark counters idempotently on pipeline runs', () => {
        ...
        const block = functionBlockFromSql(pipelineSql, 'record_user_pipeline_bookmark_terminal');
        expect(block).toContain('ON CONFLICT (pipeline_run_id, bookmark_id) DO NOTHING');
        ...
        expect(block).toContain('RETURN (terminal_count + untracked_error_count) >= total_bookmarks;');
    });

    it('records clustering enqueue only after the coordinator queues the job', () => {
        const claimBlock = functionBlockFromSql(pipelineSql, 'claim_user_pipeline_clustering');
        const enqueuedBlock = functionBlockFromSql(pipelineSql, 'record_user_pipeline_clustering_enqueued');
        const releaseBlock = functionBlockFromSql(pipelineSql, 'release_user_pipeline_clustering_claim');

        expect(claimBlock).not.toContain("jsonb_build_object('clusteringEnqueuedAt', NOW())");
        ...
        expect(claimBlock).toContain("current_run.totals->>'clusteringClaimedAt' IS NOT NULL");
        expect(enqueuedBlock).toContain("jsonb_build_object('clusteringEnqueuedAt', NOW())");
        expect(enqueuedBlock).toContain("totals - 'clusteringClaimedAt' - 'clusteringClaimId'");
        expect(enqueuedBlock).toContain("totals->>'clusteringClaimId' = p_claim_id::text");
        expect(enqueuedBlock).toContain("totals->>'clusteringEnqueuedAt' IS NULL");
        expect(releaseBlock).toContain("totals - 'clusteringClaimedAt' - 'clusteringClaimId'");
        expect(releaseBlock).toContain("totals->>'clusteringClaimId' = p_claim_id::text");
    ...
    it('keeps run telemetry out of user pipeline controls', () => {
        ...
        const untrackedBlock = functionBlockFromSql(pipelineSql, 'record_user_pipeline_untracked_error');
        const claimBlock = functionBlockFromSql(pipelineSql, 'claim_user_pipeline_clustering');
    ...
    it('deduplicates untracked errors by run and error key', () => {
        ...
        const block = functionBlockFromSql(pipelineSql, 'record_user_pipeline_untracked_error');
    ...
    it('scopes status bookmark and assignment counts to a pipeline run', () => {
        const block = functionBlockFromSql(pipelineSql, 'get_pipeline_run_status_counts');

        expect(block).toContain('COUNT(*) FILTER (WHERE status =');
        expect(block).toContain('AND pipeline_run_id = p_pipeline_run_id');
        expect(block).toContain('COUNT(DISTINCT ca.bookmark_id) AS assigned_bookmarks');
        ...
        expect(pipelineSql).toContain('REVOKE ALL ON FUNCTION public.get_pipeline_run_status_counts(UUID, UUID) FROM PUBLIC, anon, authenticated');
        expect(pipelineSql).toContain('GRANT EXECUTE ON FUNCTION public.get_pipeline_run_status_counts(UUID, UUID) TO service_role');
    });
});
