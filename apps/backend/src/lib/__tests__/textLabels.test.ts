...
describe("extractPrimaryDomainLabel", () => {
  ...
  it("uses registrable domain for multi-part public suffixes", () => {
    expect(extractPrimaryDomainLabel("https://www.bbc.co.uk/news")).toBe("bbc");
    expect(extractPrimaryDomainLabel("https://shop.example.co.uk")).toBe(
      "example",
    );
  });

  it("handles common single-suffix domains and bare hostnames", () => {
    expect(extractPrimaryDomainLabel("https://www.github.com")).toBe("github");
    expect(extractPrimaryDomainLabel("https://localhost")).toBe("localhost");
  });
});
