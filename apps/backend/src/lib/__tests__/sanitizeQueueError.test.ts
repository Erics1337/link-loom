...
describe("sanitizeQueueError", () => {
  ...
  it("truncates sanitized output to the database limit", () => {
    ...
    expect(sanitizeQueueErrorMessage(message)).toHaveLength(
      MAX_QUEUE_ERROR_MESSAGE_LENGTH,
    );
  });

  it("hashes the sanitized message and sanitizes Error instances", () => {
    const raw = "boom for user@example.com";
    const sanitized = "boom for [email]";
    const prepared = prepareQueueJobFailureError(new Error(raw));

    expect(prepared.error_message_sanitized).toBe(sanitized);
    expect(prepared.error_message_hash).toBe(
      createHash("sha256").update(sanitized).digest("hex"),
    );
    expect(prepared.error_message_hash).toBe(hashQueueErrorMessage(sanitized));
  });
});
