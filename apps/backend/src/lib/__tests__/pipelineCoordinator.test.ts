...
describe('pipelineCoordinator', () => {
    ...
    it('queues fresh clustering jobs by pipelineRunId without carrying jobGeneration', async () => {
        ...
        expect(supabase.rpc).toHaveBeenNthCalledWith(1, 'record_user_pipeline_bookmark_terminal', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_bookmark_id: 'bookmark-7',
        });
        expect(supabase.rpc).toHaveBeenNthCalledWith(2, 'claim_user_pipeline_clustering', {
            p_user_id: 'user-1',
            p_job_generation: 7,
        });
        expect(supabase.rpc).toHaveBeenNthCalledWith(3, 'record_user_pipeline_clustering_enqueued', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_claim_id: claimId,
        });
    ...
    it('does not claim clustering until staged terminal counters indicate readiness', async () => {
        ...
        expect(supabase.rpc).toHaveBeenCalledWith('record_user_pipeline_bookmark_terminal', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_bookmark_id: 'bookmark-7',
        });
    ...
    it('releases the clustering claim when queueing fails', async () => {
        (supabase.rpc as any)
            ...
            .mockResolvedValueOnce({ data: claimId, error: null })
            .mockResolvedValueOnce({ data: null, error: null });
        (queues.clustering.add as any).mockRejectedValueOnce(new Error('queue unavailable'));
        ...
        expect(supabase.rpc).toHaveBeenNthCalledWith(3, 'release_user_pipeline_clustering_claim', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_claim_id: claimId,
        });
        expect(supabase.rpc).not.toHaveBeenCalledWith('record_user_pipeline_clustering_enqueued', expect.anything());
    });
    ...
    it('retries enqueue recording and releases the claim/removes the queued job when recording keeps failing', async () => {
        ...
        expect(supabase.rpc).toHaveBeenNthCalledWith(4, 'record_user_pipeline_clustering_enqueued', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_claim_id: claimId,
        });
        expect(supabase.rpc).toHaveBeenNthCalledWith(5, 'record_user_pipeline_clustering_enqueued', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_claim_id: claimId,
        });
        expect(supabase.rpc).toHaveBeenNthCalledWith(6, 'release_user_pipeline_clustering_claim', {
            p_user_id: 'user-1',
            p_job_generation: 7,
            p_claim_id: claimId,
        });
