...
const backendUrl = (
  process.env.BACKEND_SMOKE_URL ||
  process.env.BACKEND_URL ||
  "http://127.0.0.1:3333"
).replace(/\/$/, "");
const userId = process.env.BACKEND_SMOKE_USER_ID || "";
const accessToken =
  process.env.BACKEND_SMOKE_ACCESS_TOKEN ||
  process.env.BACKEND_SMOKE_TOKEN ||
  "";
...
const result = (name: string, ok: boolean, detail: string): SmokeResult => ({
  name,
  ok,
  detail,
});
...
const smokeHealth = async () => {
  ...
  return result(
    "health",
    response.ok && (body as any)?.status === "ok",
...
const smokeStatus = async () => {
  if (!userId || !accessToken) {
    return result(
      "status",
      false,
      "BACKEND_SMOKE_USER_ID and BACKEND_SMOKE_ACCESS_TOKEN are required",
    );
  }
  ...
  return result(
    "status",
    response.ok && typeof (body as any)?.total === "number",
    `${response.status} ${JSON.stringify(body)}`,
  );
};
...
const smokeEmptyIngest = async () => {
  ...
  return result(
    "ingest-empty",
    response.ok && (body as any)?.status === "queued",
    `${response.status} ${JSON.stringify(body)}`,
  );
};
...
const main = async () => {
  ...
  for (const item of results) {
    console.log(
      `[SMOKE] ${item.ok ? "PASS" : "FAIL"} ${item.name}: ${item.detail}`,
    );
  }
