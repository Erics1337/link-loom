...
export const recoverStalePipelineState = async (
  ...
  const runKey =
    pipelineRunId ?? (jobGeneration != null ? String(jobGeneration) : "legacy");
  ...
  for (const enrichmentJob of toQueueEnrichment) {
    try {
      await queues.enrichment.add(
        ...
        {
          jobId: `enrich-${userId}-run-${runKey}-${enrichmentJob.bookmarkId}`,
        },
      );
      enrichmentQueued++;
    } catch (error) {
  ...
  for (const embeddingJob of toQueueEmbedding) {
    try {
      await queues.embedding.add(
        ...
        {
          jobId: `embed-${userId}-run-${runKey}-${embeddingJob.bookmarkId}`,
        },
      );
      embeddingQueued++;
    } catch (error) {
