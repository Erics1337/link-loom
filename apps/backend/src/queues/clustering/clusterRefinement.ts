...
import { ClusterGroup, rankIdsByCentroid } from "./clusterAlgorithm";
...
const loadBookmarkSummaries = async (
  ...
  if (error) {
    throw new Error(
      `Failed to load bookmark summaries for cluster refinement: ${error.message}`,
    );
  }
...
const buildGroupContext = async (groups: ClusterGroup[]) => {
  const representativeIdsByGroup = groups.map((group) =>
    rankIdsByCentroid(group.ids, group.vecs, CLUSTER_REFINEMENT_SAMPLE_SIZE),
  );
...
const applySafeRefinement = (
  ...
  const mergeCandidates = [...(response.merges ?? [])]
    ...
    .filter(
      ({ source, target }) => source >= 0 && target >= 0 && source !== target,
    )
  ...
  if (appliedMerges > 0) {
    log(
      `Applied ${appliedMerges} LLM sibling cluster merge${appliedMerges === 1 ? "" : "s"}`,
    );
  }
