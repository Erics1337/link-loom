...
describe("clusterRefinement", () => {
  ...
  it("applies safe LLM sibling labels and whole-group merges", async () => {
    const groups = [createGroup("ai-a", 12, 0), createGroup("ai-b", 12, 0.1)];
    ...
    const refined = await refineSiblingGroupsWithLLM(groups, settings, vi.fn());
  ...
  it("skips the LLM when the sibling group is below the configured threshold", async () => {
    const groups = [createGroup("small-a", 6, 0), createGroup("small-b", 6, 1)];
