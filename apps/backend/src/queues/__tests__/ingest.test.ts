...
describe('Ingest Worker', () => {
    ...
    const createBatchedTableMocks = ({
        ...
        const bookmarksUpsert = vi.fn().mockReturnValue({ select: bookmarksSelect });
        ...
        const clusterAssignmentsDelete = vi.fn().mockResolvedValue({ error: null });
        const clusterAssignmentsInsert = vi.fn().mockResolvedValue({ error: null });
        const mockClusterAssignmentsChain = {
            delete: vi.fn(() => ({
                eq: clusterAssignmentsDelete,
                in: clusterAssignmentsDelete
            })),
    ...
    it('should correctly process a cache MISS and enqueue to enrichment', async () => {
        ...
        expect(recordPipelineIngestTotal).toHaveBeenCalledWith('user-1', 9, 'run-9', 1);
    ...
    it('should skip an unchanged bookmark entirely (no upsert, no enrichment)', async () => {
        ...
        expect(queues.enrichment.add).not.toHaveBeenCalled();
        expect(recordPipelineIngestTotal).toHaveBeenCalledWith('user-1', 11, 'run-11', 0);
        expect(recordPipelineIngestCompleted).toHaveBeenCalledWith(
            'user-1',
            11,
            'run-11',
