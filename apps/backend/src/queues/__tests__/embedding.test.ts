...
describe("Embedding Worker", () => {
  beforeEach(() => {
    ...
    (safeFetch as any).mockResolvedValue({
      text: () =>
        Promise.resolve(
          "<html><body><main>Detailed page content for embeddings.</main></body></html>",
        ),
    });
  });
  ...
  it("should correctly process a cache MISS, call OpenAI, and cache the result", async () => {
    ...
    expect(mockBookmarksChain.eq).toHaveBeenCalledWith("id", "bm-1");
    expect(mockBookmarksChain.eq).toHaveBeenCalledWith("user_id", "user-1");

    expect(mockCreate).toHaveBeenCalledWith({
      model: "text-embedding-3-small",
      input:
        "Test title Test description Detailed page content for embeddings. https://example.com",
    });
    ...
    expect(safeFetch).toHaveBeenCalledWith("https://example.com", {
      timeoutMs: 5000,
    });
    ...
    expect(isUserCancelled).toHaveBeenCalledWith(
      "user-1",
      6,
      "run-6",
      expect.any(Object),
  ...
  it("should correctly process a cache HIT and skip OpenAI", async () => {
    ...
    const mockSharedLinksChain = createMockChain({
      data: { vector: cachedVector },
      error: null,
    }); // cache HIT
    ...
    expect(isUserCancelled).toHaveBeenNthCalledWith(
      1,
      "user-2",
      7,
      "run-7",
  ...
  it("should keep embedded status when terminal notification fails after embedding succeeds", async () => {
    ...
    await expect(embeddingProcessor(job)).rejects.toThrow("rpc unavailable");

    expect(mockCreate).not.toHaveBeenCalled();
    expect(mockBookmarksChain.update).toHaveBeenCalledWith({
      status: "embedded",
    });
    expect(mockBookmarksChain.update).not.toHaveBeenCalledWith({
      status: "error",
    });
    expect(errorSpy).toHaveBeenCalledWith(
      "[EMBEDDING] Failed to notify pipeline terminal state for bookmark bm-2 (jobGeneration=7, pipelineRunId=run-7)",
      notifyError,
    );
  ...
  it("should retry when cancellation evaluation fails", async () => {
    ...
    await expect(embeddingProcessor(job)).rejects.toThrow(
