...
describe('clusterPersistence', () => {
    ...
    it('persists distance_to_centroid only for bookmarks with a known distance', async () => {
        ...
        expect(insert).toHaveBeenCalledWith([
            { cluster_id: 'cluster-1', bookmark_id: 'bookmark-1', distance_to_centroid: 0.25 },
            { cluster_id: 'cluster-1', bookmark_id: 'bookmark-2', distance_to_centroid: 0 },
            { cluster_id: 'cluster-1', bookmark_id: 'bookmark-3' },
        ]);
    });

    it('stores keywords on the cluster, or null when there are none', async () => {
        const single = vi.fn().mockResolvedValue({ data: { id: 'cluster-1' }, error: null });
    ...
    it('returns an empty set and logs when the pinned bookmark lookup fails', async () => {
    ...
    it('clears unpinned assignments via the scoped RPC and reports success', async () => {
        ...
        const result = await clearUnpinnedAssignmentsAndPruneClusters('user-1', log);
