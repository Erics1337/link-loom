...
describe("Enrichment Worker", () => {
  ...
  it("should notify and stop processing if cancelled after fetch", async () => {
    ...
    (safeFetch as any).mockResolvedValueOnce({
      text: () =>
        Promise.resolve("<html><head><title>Fetched</title></head></html>"),
    });
    const job = createMockJob({
      userId: "user-4",
      pipelineRunId: "run-7",
  ...
  it("should use a unique embedding job id when pipeline run metadata is missing", async () => {
    (safeFetch as any).mockResolvedValueOnce({
      text: () =>
        Promise.resolve("<html><head><title>Legacy</title></head></html>"),
    });
  ...
  it("should prefer pipelineRunId over jobGeneration in the embedding job id", async () => {
    (safeFetch as any).mockResolvedValueOnce({
      text: () =>
        Promise.resolve("<html><head><title>Run</title></head></html>"),
    });
  ...
  it("should retry when pipeline notification fails after enrichment DB update error", async () => {
    ...
    (safeFetch as any).mockResolvedValueOnce({
      text: () =>
        Promise.resolve("<html><head><title>Title</title></head></html>"),
    });
    ...
    await expect(enrichmentProcessor(job)).rejects.toThrow("rpc unavailable");
  ...
  it("should retry when cancellation evaluation fails", async () => {
    ...
    await expect(enrichmentProcessor(job)).rejects.toThrow(
