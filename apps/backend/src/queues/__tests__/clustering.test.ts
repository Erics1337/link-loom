...
const createMockChain = (resolvedValue: any, explicitCount?: number) => {
  const chain: any = {
    ...
    then: (resolve: any) =>
      resolve(
        explicitCount !== undefined ? { count: explicitCount } : resolvedValue,
      ),
  };
  return chain;
};
...
describe("Clustering Worker", () => {
  ...
  it("should complete an empty run when no bookmarks are available", async () => {
    ...
    (supabase.from as any).mockImplementation((table: string) => {
      ...
      if (table === "cluster_assignments")
        return createClusterAssignmentsChain(vi.fn());
      return {};
    });

    await clusteringProcessor(job);

    expect(recordPipelineClusteringCompleted).toHaveBeenCalledWith(
      "user-1",
      11,
      "run-11",
    );
  ...
  it("should fetch valid bookmarks and create at least a root cluster", async () => {
    ...
    const mockClustersChain = {
      ...
      single: vi
        .fn()
        .mockResolvedValue({ data: { id: "cluster-123" }, error: null }),
    };
    ...
    (supabase.from as any).mockImplementation((table: string) => {
      ...
      if (table === "cluster_assignments")
        return createClusterAssignmentsChain(mockClusterAssignChain.insert);
      return {};
    });
  ...
  it("should skip bookmarks whose vector dimensions do not match the first valid vector", async () => {
    const job = createMockJob({
      userId: "user-dims",
      pipelineRunId: "run-dims",
      jobGeneration: 13,
    });
    ...
    (supabase.from as any).mockImplementation((table: string) => {
          ...
          range: vi
            ...
            .mockResolvedValueOnce({
              data: [
                ...
                {
                  id: "bm-wrong-dims",
                  shared_links: { vector: [0.3, 0.4, 0.5] },
                },
                { id: "bm-2", shared_links: { vector: [0.6, 0.7] } },
              ],
              error: null,
            })
      ...
      if (table === "clusters") {
        return {
          ...
          single: vi
            .fn()
            .mockResolvedValue({ data: { id: "cluster-dims" }, error: null }),
        };
      }
    ...
    expect(mockAssignmentInsert).not.toHaveBeenCalledWith(
      expect.arrayContaining([
        expect.objectContaining({ bookmark_id: "bm-wrong-dims" }),
      ]),
    );
    expect(completePipelineRun).toHaveBeenCalledWith(
      "run-dims",
  ...
  it("should fail the run when assignment persistence fails", async () => {
    const job = createMockJob({
      userId: "user-assign-fail",
      pipelineRunId: "run-assign-fail",
      jobGeneration: 14,
    });
    const mockAssignmentInsert = vi.fn().mockResolvedValue({
      error: { message: "assignment insert failed" },
    });
    ...
    await expect(clusteringProcessor(job)).rejects.toThrow(
      "Partial assignment failure for cluster cluster-assign-fail: inserted 0/2, failed 2",
    );
  ...
  it("should accept Supabase joined vectors returned as arrays", async () => {
    ...
    expect(mockAssignmentInsert).toHaveBeenCalledWith([
      expect.objectContaining({
        cluster_id: "cluster-array-shape",
        bookmark_id: "bm-1",
      }),
      expect.objectContaining({
        cluster_id: "cluster-array-shape",
    ...
    (supabase.from as any).mockImplementation((table: string) => {
      ...
      if (table === "clusters") {
        return {
          ...
          single: vi
            .fn()
            .mockResolvedValue({ data: { id: "root-cluster" }, error: null }),
        };
      }
  ...
  it("should not write any clusters or assignments when cancelled after the tree is built but before persistence", async () => {
    ...
    const mockClustersChain = {
      ...
      select: vi.fn().mockReturnThis(),
      single: vi.fn().mockResolvedValue({
        data: { id: "cluster-should-not-be-created" },
        error: null,
      }),
    };
    const mockClusterAssignChain = {
    ...
    expect(
      rpcCalls.some(([name]) => name === "clear_unpinned_cluster_assignments"),
    ).toBe(false);
    expect(recordPipelineClusteringCompleted).not.toHaveBeenCalled();
    expect(completePipelineRun).not.toHaveBeenCalled();
  });
  ...
  it("should throw and skip clearing when pinned bookmark lookup fails", async () => {
    const job = createMockJob({
      userId: "user-pinned-fail",
      pipelineRunId: "run-pinned-fail",
      jobGeneration: 18,
    });
    const rpcCalls: any[] = [];
    (supabase.rpc as any).mockImplementation((name: string, args: any) => {
          ...
          select: vi.fn(() => ({
            eq: vi.fn(() => ({
              eq: vi.fn(() => ({
                range: vi.fn().mockResolvedValue({
                  data: null,
                  error: { message: "pinned lookup failed" },
                }),
              })),
            })),
          })),
    ...
    await expect(clusteringProcessor(job)).rejects.toEqual({
      message: "pinned lookup failed",
    });
  ...
  it("should exclude pinned bookmarks from clustering and count them as assigned", async () => {
    ...
    const mockClustersChain = {
      ...
      single: vi
        .fn()
        .mockResolvedValue({ data: { id: "cluster-unpinned" }, error: null }),
    };
    const mockAssignmentInsert = vi.fn().mockResolvedValue({ error: null });
    const rpcCalls: any[] = [];
    (supabase.rpc as any).mockImplementation((name: string, args: any) => {
    ...
    // clear_unpinned_cluster_assignments runs immediately before persistence, leaving the pin alone.
    expect(
      rpcCalls.some(([name]) => name === "clear_unpinned_cluster_assignments"),
    ).toBe(true);
    ...
    expect(mockAssignmentInsert).toHaveBeenCalledWith([
      expect.objectContaining({
        cluster_id: "cluster-unpinned",
        bookmark_id: "bm-unpinned",
      }),
    ]);
    expect(completePipelineRun).toHaveBeenCalledWith(
      "run-pin",
  ...
  it("should skip orphaned clustering jobs when enqueue was not recorded", async () => {
    ...
    expect(shouldExecutePipelineClustering).toHaveBeenCalledWith(
      "user-7",
      17,
      "run-17",
    );
