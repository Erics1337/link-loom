...
const extractPageText = ($: cheerio.CheerioAPI) => {
  ...
  const text =
    parts.find((part) => part && part.trim().length >= 200) ??
...
const loadBookmarkMetadataText = async (userId: string, bookmarkId: string) => {
  ...
  if (error) {
    throw new Error(
      `Failed to load bookmark ${bookmarkId} for embedding: ${error.message}`,
    );
  }
...
const loadPageText = async (url: string) => {
  ...
  try {
    ...
    if (err.name === "AbortError") {
      ...
      console.warn(
        `[EMBEDDING SCRAPE FAILED] urlHash=${urlLogId}: ${err.name || "Error"}`,
      );
    }
    return "";
  }
...
const buildBookmarkEmbeddingInput = async (
  ...
  const [metadataText, pageText] = await Promise.all([
    loadBookmarkMetadataText(userId, bookmarkId),
    loadPageText(url),
  ]);
...
const notifyTerminalState = async (
  userId: string,
  jobGeneration: number | undefined,
  pipelineRunId: string | undefined,
  bookmarkId: string,
  ...
  try {
    ...
    console.error(
      `[EMBEDDING] Failed to notify pipeline terminal state for bookmark ${bookmarkId} (jobGeneration=${jobGeneration}, pipelineRunId=${pipelineRunId})`,
      notifyError,
    );
    throw notifyError;
  }
...
export const embeddingProcessor = async (job: QueueJob<EmbeddingJobData>) => {
  const { userId, pipelineRunId, jobGeneration, bookmarkId, text, url } =
    job.data;
  ...
  try {
    ...
    if (cacheLookupError) {
      throw new Error(
        `Shared link lookup failed for urlHash=${urlHash}: ${cacheLookupError.message}`,
      );
    }
    ...
    if (cached?.vector) {
      ...
      const embeddingInput = await buildBookmarkEmbeddingInput(
        userId,
        bookmarkId,
        url,
        text,
      ...
      if (sharedUpdateError) {
        throw new Error(
          `Failed to persist shared vector for urlHash=${urlHash}: ${sharedUpdateError.message}`,
        );
      }
    }
    ...
    if (bookmarkStatusError) {
      throw new Error(
        `Failed to mark bookmark ${bookmarkId} as embedded: ${bookmarkStatusError.message}`,
      );
    }
  } catch (err) {
