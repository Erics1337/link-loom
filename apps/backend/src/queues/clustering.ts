    ...
    const namedGroups = await Promise.all(
      groups.map((group) =>
        limitClusterNaming(async () => {
          const representativeIds = rankIdsByCentroid(group.ids, group.vecs);
          const { name, keywords } = await generateClusterName(
            group.ids,
            settings,
            log,
...
async function persistClusterTree(
  userId: string,
  nodes: ClusterNode[],
): Promise<{ assignedBookmarks: number }> {
  const persistNode = async (
    ...
    if (!clusterId) {
      throw new Error(
        `Unable to persist cluster "${node.name}" for user ${userId}: creation failed`,
      );
    }

    let assignedBookmarks = 0;
    if (node.bookmarkIds.length > 0) {
      ...
      if (
        ...
        node.bookmarkIds.forEach((id, index) => {
          distanceById.set(id, computeDistance(node.vectors[index], centroid));
        });
      }
      ...
      if (!assignmentResult.success) {
        throw new Error(
          `Partial assignment failure for cluster ${clusterId}: inserted ${assignmentResult.inserted}/${assignmentResult.total}, failed ${assignmentResult.failed}`,
        );
      }
      assignedBookmarks += assignmentResult.inserted;
    }

    const childAssignedCounts = await Promise.all(
      node.children.map((child) => persistNode(child, clusterId)),
    );
    assignedBookmarks += childAssignedCounts.reduce(
      (sum, count) => sum + count,
      0,
    );
    return assignedBookmarks;
  };

  const rootAssignedCounts = await Promise.all(
    nodes.map((node) => persistNode(node, null)),
  );
  return {
    assignedBookmarks: rootAssignedCounts.reduce(
      (sum, count) => sum + count,
      0,
    ),
  };
...
export const clusteringProcessor = async (job: QueueJob<ClusteringJobData>) => {
  ...
  if (
    !(await shouldExecutePipelineClustering(
      userId,
      jobGeneration,
      pipelineRunId,
    ))
  ...
  for (const bookmark of userBookmarks) {
    ...
    if (expectedVectorDimensions === null) {
      expectedVectorDimensions = parsedVector.length;
    } else if (parsedVector.length !== expectedVectorDimensions) {
      log(
        `Skipping bookmark ${bookmark.id}: vector dimension ${parsedVector.length} does not match expected ${expectedVectorDimensions}`,
      );
      continue;
    }
  ...
  const persistResult = await persistClusterTree(userId, clusterTree);

  await finalizePipelineRun(userId, jobGeneration, pipelineRunId, {
    totalBookmarks: userBookmarks.length,
    embeddedBookmarks: parsedRows.length,
    assignedBookmarks: pinnedBookmarkIds.size + persistResult.assignedBookmarks,
  });
