...
const pinBookmarkToCluster = async (bookmarkId: string, clusterId: string) => {
  ...
  console.error(
    `[INGEST WORKER] Failed to replace existing cluster assignment for bookmark ${bookmarkId}:`,
    deleteError,
  );
  ...
  if (rollbackError) {
    console.error(
      `[INGEST WORKER] Failed to roll back pin for bookmark ${bookmarkId} in cluster ${clusterId}:`,
      rollbackError,
    );
  }
};
...
const ensureClusterForChromeFolder = async (
  ...
  pendingClusterIdByChromeFolderId: Map<string, Promise<string | null>>,
): Promise<string | null> => {
  const existing = clusterIdByChromeFolderId.get(chromeFolderId);
  if (existing) return existing;

  const pending = pendingClusterIdByChromeFolderId.get(chromeFolderId);
  ...
  pendingClusterIdByChromeFolderId.set(chromeFolderId, creation);

  try {
    return await creation;
  } finally {
    if (pendingClusterIdByChromeFolderId.get(chromeFolderId) === creation) {
      pendingClusterIdByChromeFolderId.delete(chromeFolderId);
    }
  }
};
...
export const ingestProcessor = async (job: QueueJob<IngestJobData>) => {
  ...
  try {
    ...
    const clusterIdByChromeFolderId =
      await fetchClusterIdsByChromeFolderId(userId);
    const pendingClusterIdByChromeFolderId = new Map<
      string,
      Promise<string | null>
    >();
    ...
    const classifications: Classified[] = dedupedBookmarks.map((item) => {
      ...
      if (existing.url !== item.url)
        return { kind: "urlChanged", item, existing };
    ...
    const urlChangedExistingIds = newOrUrlChanged
      .filter(
        (c): c is Extract<NewOrUrlChanged, { kind: "urlChanged" }> =>
          c.kind === "urlChanged",
      )
      .map((c) => c.existing.id);
        ...
        limitMetadata(async () => {
          ...
          const clusterId =
            ...
            (await ensureClusterForChromeFolder(
              ...
              pendingClusterIdByChromeFolderId,
            ));
    ...
    for (
      ...
      const { data: insertedRows, error: bookmarkUpsertError } = await supabase
        .from("bookmarks")
        .upsert(
          items.map((item) => ({
            user_id: userId,
      ...
      await Promise.all(
        cacheMisses.map(({ row }) =>
          limitNotify(() => {
            const runKey =
              pipelineRunId ??
              (jobGeneration != null ? String(jobGeneration) : "legacy");

            return queues.enrichment.add(
              "enrich",
              {
                userId,
                pipelineRunId,
              ...
              {
                jobId: `enrich-${userId}-run-${runKey}-${row.id}`,
              },
            );
          }),
        ),
