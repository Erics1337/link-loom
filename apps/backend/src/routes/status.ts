...
const getTotalsNumber = (
  run: PipelineRunRow | null | undefined,
  key: string,
) => {
...
const buildStatusResponse = ({
  ...
  const pipelineStatus =
    currentRun?.status == null ? null : String(currentRun.status);
  ...
  return {
    ...
    pipelineGeneration:
      currentRun?.generation == null ? null : Number(currentRun.generation),
    pipelineStatus,
  };
};
...
const loadLegacyStatusCounts = async (userId: string) => {
  ...
  const remainingToAssign = Math.max(
    counts.embeddedBookmarks - counts.assignedBookmarks,
    0,
  );
    ...
    async (req, reply) => {
      ...
      const [
        ...
        supabase
          .from("pipeline_runs")
          .select(
            "id, generation, status, started_at, cancelled_at, completed_at, settings, totals, error_summary",
          )
          .eq("user_id", userId)
      ...
      if (controlError)
        console.error("[STATUS] Pipeline Control Error:", controlError);
      if (latestRunError)
        console.error("[STATUS] Latest Run Error:", latestRunError);

      const controlRunId =
        control?.current_pipeline_run_id == null
          ? null
      ...
      if (controlRunId && controlRunId !== latestRun?.id) {
        ...
        if (controlRunError)
          console.error("[STATUS] Current Run Error:", controlRunError);
