...
export const registerToolRoutes = async (fastify: FastifyInstance) => {
  fastify.post(
    ...
    async (req, reply) => {
      ...
      console.log(
        `[DEAD_LINKS] Checking ${uniqueUrls.length} unique URLs (from ${bookmarks.length} bookmarks)`,
      );
      ...
      const deadChromeIds = Array.from(deadUrlSet).flatMap(
        (url) => chromeIdsByUrl.get(url) ?? [],
      );
      console.log(
        `[DEAD_LINKS] Done: scanned=${scannedCount}, dead=${deadChromeIds.length}, skipped=${skipped}`,
      );
      return {
  ...
  fastify.post(
    ...
    async (req, reply) => {
      ...
      try {
        ...
        console.error(
          "[AUTO_RENAME] Failed to load assignments",
          assignmentError,
        );
        return reply
          .code(500)
          .send({ error: "Failed to load bookmarks for rename" });
      }
      ...
      await Promise.all(
        contexts.map((context) =>
          renameLimit(async () => {
            ...
            if (normalizedSuggestion === currentTitle && !currentAiTitle)
              return;
            ...
            updates.push({
              bookmark_id: context.bookmarkId,
              ai_title: normalizedSuggestion,
            });
          }),
        ),
      );
