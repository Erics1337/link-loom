...
describe('status routes', () => {
    ...
    it('uses run-scoped counts for a running pipeline run', async () => {
        ...
        expect(supabase.rpc).toHaveBeenCalledWith('get_pipeline_run_status_counts', {
            p_user_id: 'user-1',
            p_pipeline_run_id: 'run-1',
        });
        expect((supabase.from as any).mock.calls.map(([table]: [string]) => table)).not.toContain('bookmarks');
    });
    ...
    it.each(['cancelled', 'failed'])('treats %s runs as terminal without marking them completed', async (status) => {
        const run = {
            id: `run-${status}`,
            generation: 9,
            status,
