...
const snapshotMutationHandler =
  ...
  async (req: any, reply: any) => {
    ...
    return runSnapshotMutation(reply, logLabel, failureMessage, status, () =>
      ...
      try {
        const { data: snapshotId, error } = await supabase.rpc(
          "create_structure_snapshot",
          {
            p_user_id: userId,
            p_snapshot_name:
              name || `Cloud Snapshot ${new Date().toISOString().slice(0, 10)}`,
          },
        );
    ...
    snapshotMutationHandler(
      ...
      async (userId, snapshotId) => {
        ...
        try {
          ...
          if (error) throw error;
          await completePipelineRun(pipelineRun.id);
        } catch (error) {
