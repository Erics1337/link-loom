...
const densityOptions: Array<{
...
}> = [
  {
    value: "less",
    label: "Less folders",
    hint: "Bigger groups with broader categories.",
  },
  { value: "medium", label: "Medium", hint: "Balanced depth and findability." },
  {
    value: "more",
    label: "More folders",
    hint: "Smaller groups with finer topic splits.",
  },
];
...
const toneOptions: Array<{ value: NamingTone; label: string; hint: string }> = [
  {
    value: "clear",
    label: "Clear",
    hint: "Literal and straightforward names.",
  },
  {
    value: "balanced",
    label: "Balanced",
    hint: "Mostly direct with slight personality.",
  },
  {
    value: "playful",
    label: "Playful",
    hint: "Creative names with topical anchors.",
  },
];
    ...
    <div className="app-shell">
      <div className="app-header">
        <button
          ...
          className="btn-icon"
          title="Go Back"
          disabled={accountMessage === "Account deleted."}
        >
          <ArrowLeft size={18} />
        </button>
          ...
          <div className="card space-y-4">
            <div>
              ...
              <p className="text-xs text-secondary mb-3">
                {
                  densityOptions.find(
                    (option) => option.value === settings.folderDensity,
                  )?.hint
                }
              </p>
              <CustomSelect
                ...
                onChange={(val) =>
                  onSettingsChange({ folderDensity: val as FolderDensity })
                }
          ...
          <div className="card space-y-4">
            <div>
              ...
              <p className="text-xs text-secondary mb-3">
                {
                  toneOptions.find(
                    (option) => option.value === settings.namingTone,
                  )?.hint
                }
              </p>
              <CustomSelect
                ...
                onChange={(val) =>
                  onSettingsChange({ namingTone: val as NamingTone })
                }
                options={toneOptions}
              />
            </div>
            ...
            >
              <div className="text-left pr-4">
                ...
                <span className="block text-xs text-secondary mt-1 leading-relaxed">
                  Adds emoji prefixes to AI generated folder names.
                </span>
              </div>
              <div className="toggle mt-0.5">
                <input
                  ...
                  onChange={(event) =>
                    onSettingsChange({ useEmojiNames: event.target.checked })
                  }
                />
            ...
            >
              <div>
                ...
                <span className="block text-xs text-secondary mt-1">
                  Uses compact extension palette.
                </span>
              </div>
            ...
            <div className="card space-y-3">
              <div>
                <div className="text-sm font-bold mb-1">
                  Delete Link Loom Account
                </div>
                <p className="text-xs text-secondary">
                  Removes your Link Loom account and cloud data
                  {accountEmail ? ` for ${accountEmail}` : ""}. Bookmarks
                  already stored in Chrome remain in this browser.
                </p>
              </div>
              {accountMessage && (
                <p className={`text-xs ${accountMessage === "Account deleted." ? "text-success" : "text-danger"}`}>
                  {accountMessage}
                </p>
              )}
              <button
                ...
                disabled={isDeletingAccount || accountMessage === "Account deleted."}
              >
                <Trash2 size={14} />
