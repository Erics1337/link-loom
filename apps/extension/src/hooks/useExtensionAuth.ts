...
const savePendingAuthNonce = async (nonce: string) => {
  if (typeof chrome === "undefined" || !chrome.storage?.local) return;
  await chrome.storage.local.set({ [PENDING_AUTH_NONCE_STORAGE_KEY]: nonce });
};

const clearPendingAuthNonce = (nonce: string) => {
  if (typeof chrome === "undefined" || !chrome.storage?.local) return;

  chrome.storage.local.get([PENDING_AUTH_NONCE_STORAGE_KEY], (result) => {
    if (result[PENDING_AUTH_NONCE_STORAGE_KEY] === nonce) {
  ...
  const signInWithGoogle = useCallback(async () => {
    ...
    return new Promise<ExtensionAuthUser>((resolve, reject) => {
      ...
      chrome.tabs.create({ url: authUrl }, () => {
        ...
        reject(
          new Error(
            runtimeError.message || "Could not open Google sign in tab.",
          ),
        );
      });
    });
