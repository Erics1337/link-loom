...
export const useChromeApply = ({
  ...
  const [activeJournal, setActiveJournal] = useState<ChromeApplyJournal | null>(
    null,
  );
  ...
  const rollbackActiveJournal = useCallback(async () => {
    ...
    setStatus("weaving");
    try {
      const rollbackResult = await rollbackChromeBookmarkApplyJournal(journal);
      setActiveJournal(null);
      setRecoveryMessage({
        kind:
          rollbackResult.skippedDeletedFolderCount > 0 ? "error" : "success",
        text:
