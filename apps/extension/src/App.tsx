...
const App = () => {
  ...
  const {
    ...
    updateSettings: updateClusteringSettings,
  } = useClusteringSettings();
  const {
    status,
  ...
  const [view, setView] = useState<
    "main" | "settings" | "login" | "cloud-snapshots" | "import-preview"
  >("main");
  const [cloudSnapshots, setCloudSnapshots] = useState<CloudSnapshot[]>([]);
  const [cloudSnapshotsError, setCloudSnapshotsError] = useState<string | null>(
    null,
  );
  const [isImportingStructure, setIsImportingStructure] = useState(false);
  const [isApplyingImportedStructure, setIsApplyingImportedStructure] =
    useState(false);
  ...
  const { authStatus, errorMsg } = useDeviceAuth(
    isPermanentUser ? authUser?.id || "" : "",
    accessToken,
  );
  ...
  const handleOpenCloudSnapshots = async () => {
    ...
    try {
      ...
      setCloudSnapshotsError(
        error instanceof Error
          ? error.message
          : "Failed to load Cloud Snapshots.",
      );
    }
  };
  ...
  const handleSignUp = async (
    ...
    if (plan === "paid") {
      if (result.requiresEmailConfirmation) {
        throw new Error(
          "Confirm your email first, then sign in and start Pro checkout.",
        );
      }
  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      // Schedule cleanup and redirection after a 2-second delay
      // to allow SettingsScreen to render the success state.
      setTimeout(async () => {
        try {
          ...
          console.error("Failed to clean up after account deletion:", err);
        }
      }, 2000);
    } catch (error) {
  ...
  const handleImportStructure = async (file: File) => {
    ...
    try {
      ...
      setImportPreview({
        ...
        parsedExport,
        summary,
        nodes: parsedBookmarkExportToPreviewNodes(
          parsedExport,
        ) as BookmarkNode[],
      });
      ...
      setImportStructureMessage({
        ...
        text:
          error instanceof Error
            ? error.message
            : "Failed to load bookmark export.",
      });
    } finally {
  ...
  const handleApplyImportedStructure = async () => {
    ...
    try {
      ...
      const appliedSummary = await applyParsedBookmarkExport(
        importPreview.parsedExport,
        applyOptions,
      );
      ...
      setImportPreviewMessage({
        ...
        text:
          error instanceof Error
            ? error.message
            : "Failed to apply imported structure.",
      });
    } finally {
  ...
  const renderContent = () => {
        ...
        <div className="app-shell">
          <div className="panel">
            ...
            <p className="screen-copy mt-3">
              {errorMsg || "You have exceeded the maximum number of devices."}
            </p>
          </div>
          <a
            href={`${WEB_APP_URL}/dashboard/devices`}
            target="_blank"
            rel="noreferrer"
    ...
    if (
      status === "idle" &&
      (extensionAuthStatus === "loading" || isCheckingInitialStatus)
    ) {
      return (
        <WeavingScreen
          progress={5}
          eyebrow="Processing"
          title="Checking current run"
          description="Link Loom is reconnecting to your bookmark scan."
          statusMessage="Looking for an active bookmark scan..."
          footerMessage="Reopen popup anytime to check progress."
        />
      );
    }
    ...
    switch (status) {
      ...
      case "weaving":
        ...
        let statusDetail = "";
        const totalBookmarks = Math.max(
          progress.total || progress.ingestTotal || 0,
          0,
        );
        ...
        if (weavingPhase === "safety-backup") {
          ...
          statusDetail =
            "Your current Chrome bookmark tree is being saved locally before organizing begins.";
        } else if (
          ...
          const ingestTotal = Math.max(
            progress.ingestTotal || progress.total || 0,
            1,
          );
          const ingestProcessed = Math.min(
            progress.ingestProcessed || 0,
            ingestTotal,
          );
          progressPercent = Math.max(
            8,
            Math.min((ingestProcessed / ingestTotal) * 45, 45),
          );
          statusMessage = `Stage 1/3: Indexing ${ingestProcessed} of ${ingestTotal} bookmarks...`;
          statusDetail =
            "Scanning links, checking cache hits, and queueing uncached pages.";
        } else if (totalBookmarks > 0 && progress.pending > 0) {
          const embedPercent = Math.min((safeEmbedded / safeTotal) * 100, 100);
          progressPercent = Math.max(
            45,
            Math.min(45 + embedPercent * 0.45, 90),
          );
          statusMessage =
            "Stage 2/3: Enriching pages and generating embeddings...";
          ...
          if (safeAssigned > 0) {
            ...
            progressPercent = Math.max(
              90,
              Math.min(90 + assignedPercent * 0.09, 99),
            );
            statusMessage = "Stage 3/3: Structuring bookmarks into folders...";
            const remaining = Math.max(
              progress.remainingToAssign ||
                totalBookmarks - safeAssigned - (progress.errored || 0),
              0,
            );
        ...
        return (
          <div className="app-shell">
            <div className="panel">
              ...
              <p className="screen-copy mt-3">
                Your bookmarks have been organized.
              </p>
              {authUser && (
                <p className="screen-copy mt-2 text-xs text-secondary">
                  A Cloud Snapshot was saved before applying — you can restore
                  it anytime from Cloud Snapshots.
                </p>
              )}
            </div>
            {cloudSnapshotsError && (
              <div className="message message-error mb-3 mx-4">
                {cloudSnapshotsError}
              </div>
            )}
            {authUser && (
              <button
                onClick={() => void handleOpenCloudSnapshots()}
                className="btn btn-secondary"
              >
                View Cloud Snapshots
              </button>
            )}
            <button
              onClick={() => {
              ...
              <a
                href={
                  accessToken && refreshToken
                    ? `${WEB_APP_URL}/dashboard/billing?access_token=${accessToken}&refresh_token=${refreshToken}`
        ...
        return (
          <div className="app-shell">
            <div className="panel">
              ...
              <h1 className="screen-title mt-2">
                You have {total.toLocaleString()} bookmarks
              </h1>
              <p className="screen-copy mt-3">
                Free preview organizes {limit.toLocaleString()} bookmarks now.
                The remaining {extra.toLocaleString()} stay exactly where they
                are.
              </p>
            </div>
