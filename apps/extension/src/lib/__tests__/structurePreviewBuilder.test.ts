...
describe("buildStructurePreview", () => {
  ...
  it("surfaces top-3 cluster keywords and flags the farthest bookmarks as low confidence", () => {
    ...
    const result = buildStructurePreview({
      ...
      bookmarkRootMap: Object.fromEntries(
        assignments.map((assignment) => [
          assignment.bookmarks.chrome_id,
          "Bookmarks Bar",
        ]),
      ),
      bookmarkPreferredRootMap: {},
    ...
    expect(
      bookmarkNodes.find((node) => node.chromeId === "chrome-0")?.lowConfidence,
    ).toBeUndefined();
  });
  ...
  it("keeps overflow bookmarks under Other Bookmarks with a badge", () => {
    ...
    const otherBookmarks = result.rootNodes.find(
      (node) => node.rootTitle === "Other Bookmarks",
    );
  ...
  it("routes bookmarks with unmatched cluster_id to overflow", () => {
    const result = buildStructurePreview({
      data: {
        clusters: [],
        assignments: [
          {
            ...
            bookmark_id: "bookmark-unmatched",
            bookmarks: {
              title: "Orphaned Bookmark",
              url: "https://orphaned.example",
              chrome_id: "chrome-orphaned",
            },
          },
        ],
      },
      availableRoots: ["Bookmarks Bar", "Other Bookmarks"],
      bookmarkRootMap: { "chrome-orphaned": "Other Bookmarks" },
      bookmarkPreferredRootMap: {},
      overflowBookmarks: [],
      originalTree: [
        {
    ...
    expect(result.assignmentSummaries).toHaveLength(0);
