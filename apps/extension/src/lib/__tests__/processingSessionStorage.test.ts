...
describe("processingSessionStorage", () => {
  ...
  it("propagates failure when chrome.storage.local.set fails", async () => {
    const storage = {
      set: vi.fn(async () => {
    ...
    await expect(savePreOrganizeBackup([{ id: "root" }])).rejects.toThrow(
      "Safety backup failed: Quota exceeded",
    );
  });
});
