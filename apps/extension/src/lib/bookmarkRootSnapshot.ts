...
const inferPreferredRootFromAncestors = (
  ...
  const hasImportedAncestor = ancestorTitles.some((title) =>
    IMPORTED_FOLDER_PATTERN.test(title),
  );
...
export const buildBookmarkRootSnapshot = (
  tree: any[],
  ...
  const topLevelNodes = Array.isArray(tree?.[0]?.children)
    ? tree[0].children
    : [];
  ...
  topLevelNodes.forEach((node: any) => {
    const rootTitle =
      ROOT_TITLES.find(
        (candidate) =>
          node.id === ROOT_IDS[candidate] || node.title === candidate,
      ) ??
    ...
    const visit = (entry: any, ancestorTitles: string[] = []) => {
      if (entry.url) {
        ...
        preferredRoots[entry.id] = inferPreferredRootFromAncestors(
          ancestorTitles,
          rootTitle,
        );
      }
      if (Array.isArray(entry.children)) {
        const nextAncestorTitles = entry?.title
          ? [...ancestorTitles, String(entry.title)]
          : ancestorTitles;
