...
export const ROOT_TITLES: BookmarkRootTitle[] = [
  "Bookmarks Bar",
  "Other Bookmarks",
  "Mobile Bookmarks",
];
...
const getDirectChildByTagName = (parent: Element, tagName: string) =>
  Array.from(parent.children).find(
    (child) => child.tagName.toUpperCase() === tagName.toUpperCase(),
  ) ?? null;
  ...
  for (let index = 0; index < children.length; index += 1) {
    ...
    if (folderHeading) {
      ...
      const nestedListElement =
        ...
        (children[index + 1]?.tagName.toUpperCase() === "DL"
          ? children[index + 1]
          : null);
      const nestedList =
...
export const parseBookmarkExportText = (
  fileText: string,
  ...
  if (parserError) {
    throw new Error(
      "The selected file could not be parsed as a bookmark export.",
    );
  }
  ...
  if (Object.keys(roots).length === 0) {
    throw new Error(
      "No Chrome bookmark roots were found in the selected export.",
    );
  }
...
export const parseBookmarkExportFile = async (
  file: File,
): Promise<ParsedBookmarkExport> => {
...
export const summarizeBookmarkExport = (
  parsed: ParsedBookmarkExport,
): BookmarkImportSummary => {
  const importedRoots = ROOT_TITLES.filter((rootTitle) =>
    Array.isArray(parsed.roots[rootTitle]),
  );
...
export const getBookmarkRootAvailability = async (): Promise<
  Record<BookmarkRootTitle, boolean>
> => {
...
export const parsedBookmarkExportToPreviewNodes = (
  ...
  ROOT_TITLES.filter((rootTitle) => Array.isArray(parsed.roots[rootTitle])).map(
    (rootTitle) => ({
      id: `import-root-${rootTitle}`,
      title: rootTitle,
      children: toPreviewNodes(
        parsed.roots[rootTitle] || [],
        `import-root-${rootTitle}`,
      ),
    }),
  );
...
const createBookmarkNodes = async (
  parentId: string,
  nodes: BookmarkImportNode[],
) => {
...
const removeExistingFolderChildrenByTitle = async (
  parentId: string,
  folderTitle: string,
) => {
  const children = await chrome.bookmarks.getChildren(parentId);
  const matchingFolders = children.filter(
    (child) => !child.url && child.title === folderTitle,
  );
...
export const applyParsedBookmarkExport = async (
  ...
  if (summary.importedRoots.length === 0) {
    throw new Error(
      "The selected export does not contain any supported Chrome bookmark roots.",
    );
  }
  ...
  if (summary.importedRoots.includes("Bookmarks Bar")) {
    ...
    await createBookmarkNodes(
      ROOT_IDS["Bookmarks Bar"],
      parsed.roots["Bookmarks Bar"] || [],
    );
  }
  ...
  if (summary.importedRoots.includes("Other Bookmarks")) {
    ...
    await createBookmarkNodes(
      ROOT_IDS["Other Bookmarks"],
      parsed.roots["Other Bookmarks"] || [],
    );
  }
  ...
  if (summary.importedRoots.includes("Mobile Bookmarks")) {
    ...
    if (rootAvailability["Mobile Bookmarks"]) {
      ...
      await removeExistingFolderChildrenByTitle(
        otherRootId,
        "Mobile Bookmarks",
      );
