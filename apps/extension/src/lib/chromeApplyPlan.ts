...
export type ChromeApplyResult = {
  ...
  folderChromeIdsByClusterId: Array<{
    clusterId: string;
...
export const loadActiveChromeApplyJournal = async () => {
  ...
  const journal = await readStorageValue<ChromeApplyJournal>(
    APPLY_JOURNAL_STORAGE_KEY,
  );
...
const normalizeTitle = (title?: string) =>
  (title || "Untitled Folder").trim() || "Untitled Folder";
...
const collectPlannedBookmarkChromeIds = (
  nodes: PlannedNode[],
  keepIds: Set<string>,
) => {
...
const collectTopLevelBookmarkChromeIds = (
  nodes: PlannedNode[],
  keepIds: Set<string>,
) => {
  for (const node of nodes) {
...
const getCleanupDeleteTargets = async (
  rootId: string,
  keepIds: Set<string>,
) => {
...
export const buildChromeBookmarkApplyPlan = async (
  rootNodes: RootNode[],
): Promise<ChromeApplyPlan> => {
  ...
  for (const rootNode of rootNodes) {
    ...
    const children = collectPlannedNodes(
      rootNode.children || [],
      summary,
      preview,
      `root-${rootNode.rootTitle}`,
    );
    const plannedBookmarkIds = new Set<string>();
    collectPlannedBookmarkChromeIds(children, plannedBookmarkIds);
    const cleanupDeleteTargets = await getCleanupDeleteTargets(
      rootId,
      plannedBookmarkIds,
    );
...
const getCreateFolderMarkerTitle = (
  journalId: string,
  planId: string,
  title: string,
...
const findCreateFolderEntry = (journal: ChromeApplyJournal, planId: string) =>
  journal.entries.find(
    (
      entry,
    ): entry is Extract<ChromeApplyJournalEntry, { type: "createFolder" }> =>
      entry.type === "createFolder" && entry.planId === planId,
  );

const findUpdateBookmarkEntry = (
  ...
  journal.entries.find(
    (
      entry,
    ): entry is Extract<ChromeApplyJournalEntry, { type: "updateBookmark" }> =>
      entry.type === "updateBookmark" && entry.chromeId === chromeId,
  );

const findMoveBookmarkEntry = (journal: ChromeApplyJournal, chromeId: string) =>
  journal.entries.find(
    (
      entry,
    ): entry is Extract<ChromeApplyJournalEntry, { type: "moveBookmark" }> =>
      entry.type === "moveBookmark" && entry.chromeId === chromeId,
  );

const findDeleteRootChildEntry = (
  journal: ChromeApplyJournal,
  chromeId: string,
) =>
  journal.entries.find(
    (
      entry,
    ): entry is Extract<ChromeApplyJournalEntry, { type: "deleteRootChild" }> =>
      entry.type === "deleteRootChild" && entry.target.chromeId === chromeId,
  );
...
const createFoldersForNodes = async (
  ...
  for (const node of nodes) {
    ...
    if (journal.createdFolderIdsByPlanId[node.planId]) {
      ...
      try {
        const folder = await findBookmarkNode(folderId);
        if (folder && folder.title !== node.title) {
          await chrome.bookmarks.update(folderId, { title: node.title });
        }
      } catch (error) {
        console.warn(
          `[ApplyChanges] Failed to update folder title for resumed folder ${node.title}:`,
          error,
        );
      }
      await createFoldersForNodes(node.children, folderId, journal, result);
      continue;
    }
    ...
    try {
      ...
      const markerTitle = getCreateFolderMarkerTitle(
        journal.id,
        node.planId,
        node.title,
      );
      const existingMarkedFolder = !entry.chromeId
        ? await findChildByTitle(parentId, markerTitle)
        : null;
      const folder =
        existingMarkedFolder ||
        (await chrome.bookmarks.create({ parentId, title: markerTitle }));
      ...
      await writeJournal(journal);
      console.error(
        `[ApplyChanges] Failed to create folder ${node.title}`,
        error,
      );
    }
  }
...
const applyBookmarksForNodes = async (
  ...
  for (const node of nodes) {
    ...
    try {
      ...
      let updateEntry = findUpdateBookmarkEntry(journal, node.chromeId);
      if (updateEntry && updateEntry.status === "applied") {
        // already applied
      } else if (nextTitle && nextTitle !== currentTitle) {
        if (!updateEntry) {
          updateEntry = {
            type: "updateBookmark",
            status: "pending",
          ...
          journal.entries.push(updateEntry);
          await writeJournal(journal);
        }
        await chrome.bookmarks.update(node.chromeId, { title: nextTitle });
        updateEntry.status = "applied";
        result.renamedCount += 1;
        await writeJournal(journal);
      }

      let moveEntry = findMoveBookmarkEntry(journal, node.chromeId);
      if (moveEntry && moveEntry.status === "applied") {
        // already applied
      } else if (current.parentId !== parentId) {
        if (!moveEntry) {
          moveEntry = {
            type: "moveBookmark",
            status: "pending",
          ...
          journal.entries.push(moveEntry);
          await writeJournal(journal);
        }
        await chrome.bookmarks.move(node.chromeId, { parentId });
        moveEntry.status = "applied";
        await writeJournal(journal);
        result.movedCount += 1;
      }
    } catch (error) {
      ...
      await writeJournal(journal);
      console.warn(
        `[ApplyChanges] Failed to move bookmark ${node.chromeId}:`,
        error,
      );
    }
  }
...
const countPlannedBookmarks = (nodes: PlannedNode[]): number =>
  nodes.reduce(
    (sum, node) =>
      sum +
      (node.kind === "bookmark" ? 1 : countPlannedBookmarks(node.children)),
    0,
  );

const collectRootKeepIds = (
  nodes: PlannedNode[],
  journal: ChromeApplyJournal,
) => {
...
const toSubtreeSnapshot = (
  node: chrome.bookmarks.BookmarkTreeNode,
): BookmarkSubtreeSnapshot => ({
  chromeId: node.id,
...
const getRescueFolderPlanId = (rootId: string) =>
  `rescue-unorganized-${rootId}`;

const ensureUnorganizedFolder = async (
  rootId: string,
  journal: ChromeApplyJournal,
  result: ChromeApplyResult,
  deleteTargetChromeIds?: Set<string>,
): Promise<string> => {
  ...
  if (knownId && (await findBookmarkNode(knownId))) {
    if (!deleteTargetChromeIds || !deleteTargetChromeIds.has(knownId)) {
      return knownId;
    }
  }
  ...
  if (
    existing &&
    (!deleteTargetChromeIds || !deleteTargetChromeIds.has(existing.id))
  ) {
    ...
    const alternative = children.find(
      (child) =>
        !child.url &&
        child.title === UNORGANIZED_FOLDER_TITLE &&
        (!deleteTargetChromeIds || !deleteTargetChromeIds.has(child.id)),
    );
    if (alternative) {
      journal.createdFolderIdsByPlanId[planId] = alternative.id;
      await writeJournal(journal);
...
const cleanupRootChildren = async (
  ...
  const deleteTargetChromeIds = new Set(
    root.cleanupDeleteTargets.map((t) => t.chromeId),
  );
  ...
  const currentRootChildIds = new Set(
    (await chrome.bookmarks.getChildren(rootId)).map((child) => child.id),
  );
  ...
  for (const target of [...deleteTargets].reverse()) {
    try {
      let deleteEntry = findDeleteRootChildEntry(journal, target.chromeId);
      if (deleteEntry && deleteEntry.status === "applied") {
        continue;
      }
      ...
      if (target.kind === "bookmark") {
        ...
        if (!current) {
          if (deleteEntry) {
            deleteEntry.status = "applied";
            await writeJournal(journal);
          }
          continue;
        }
        const unorganizedId = await ensureUnorganizedFolder(
          rootId,
          journal,
          result,
          deleteTargetChromeIds,
        );
      ...
      if (!deleteEntry) {
        ...
        journal.entries.push(deleteEntry);
        await writeJournal(journal);
      }
      ...
      await writeJournal(journal);
      console.warn(
        `[ApplyChanges] Failed to clean root child ${target.title}`,
        error,
      );
    }
  }
...
export const applyChromeBookmarkPlan = async (
  plan: ChromeApplyPlan,
): Promise<ChromeApplyResult> => {
...
const reconcileJournal = async (journal: ChromeApplyJournal) => {
  let changed = false;
  for (const entry of journal.entries) {
    ...
    if (entry.type === "updateBookmark") {
      ...
      if (existing && existing.title.trim() === entry.nextTitle.trim()) {
        entry.status = "applied";
        changed = true;
      }
    } else if (entry.type === "moveBookmark") {
      const existing = await findBookmarkNode(entry.chromeId);
      if (existing && existing.parentId === entry.nextParentId) {
        entry.status = "applied";
        changed = true;
      }
    } else if (entry.type === "deleteRootChild") {
      const existing = await findBookmarkNode(entry.target.chromeId);
      if (!existing) {
        entry.status = "applied";
        changed = true;
      }
...
const executeChromeApplyJournal = async (
  journal: ChromeApplyJournal,
): Promise<ChromeApplyResult> => {
  await reconcileJournal(journal);
...
export const resumeChromeBookmarkApplyJournal = async (
  journal: ChromeApplyJournal,
) => {
...
const rollbackAppliedJournalEntry = async (
  ...
  if (entry.type === "deleteRootChild") {
    const existing = await chrome.bookmarks
      .get(entry.target.chromeId)
      .catch(() => []);
    if (existing.length === 0) {
      if (entry.target.kind === "folder") {
        if (entry.snapshot) {
          await restoreSubtreeSnapshot(
            entry.snapshot,
            entry.rootId,
            restoredIds,
          );
  ...
  if (entry.type === "updateBookmark") {
    const existing = await findBookmarkNode(entry.chromeId);
    if (!existing) return;
    await chrome.bookmarks.update(entry.chromeId, {
  ...
  if (entry.type === "createFolder") {
    ...
    const existing = await findBookmarkNode(entry.chromeId);
    if (!existing) return;
    const children = await chrome.bookmarks
      .getChildren(entry.chromeId)
      .catch(() => []);
...
export const rollbackChromeBookmarkApplyJournal = async (
  journal: ChromeApplyJournal,
): Promise<ChromeApplyRollbackResult> => {
