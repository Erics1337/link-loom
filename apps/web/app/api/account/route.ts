...
const withCors = (request: Request, response: NextResponse) => {
...
const runDeletionStep = async (
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    lastError = error
    console.warn(`[Account Deletion] ${label} failed on attempt ${attempt}`, error)
    if (attempt < 3) {
...
export async function DELETE(request: Request) {
  const { supabase, user, response: unauthorizedResponse } = await getRequestUser(request)
  ...
  try {
    ...
    return withCors(
      request,
      sanitizeApiError('[Account Deletion] Error:', error, 'Failed to delete account')
    )
  }
}
