...
function LogoMark({ inverse = false }: { inverse?: boolean }) {
    return (
        <div className="flex items-center gap-3">
            <div className="relative h-9 w-9 overflow-hidden rounded-lg border border-[color:var(--ll-border)] bg-[var(--ll-surface-solid)] shadow-sm">
                <Image src="/logo-transparent.png" alt="Link Loom" fill sizes="36px" className="object-contain p-1" />
            </div>
            <span className={`text-lg font-semibold tracking-tight ${inverse ? 'text-[var(--ll-deep-text)]' : 'text-[var(--ll-text)]'}`}>
                Link Loom
            </span>
        </div>
    )
    ...
    return (
        <CardContainer containerClassName="py-0" className="w-full">
            <CardBody className="relative h-auto w-full max-w-[620px]">
                <CardItem translateZ={18} className="absolute -left-6 top-16 hidden h-28 w-28 border border-[color:var(--ll-primary)] md:block">
                    <span className="sr-only">Depth guide</span>
                </CardItem>
                <CardItem translateZ={28} className="absolute -right-4 bottom-12 hidden h-36 w-24 border border-[color:var(--ll-accent)] md:block">
                    <span className="sr-only">Depth guide</span>
                </CardItem>

                <CardItem translateZ={42} className="relative w-full border border-[color:var(--ll-border)] bg-[var(--ll-card)] shadow-[0_28px_80px_var(--ll-shadow)]">
                    <div className="flex items-center justify-between border-b border-[color:var(--ll-border)] px-4 py-3">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="h-2.5 w-2.5 rounded-full bg-[#d2a64a]" />
                            <span className="h-2.5 w-2.5 rounded-full bg-[var(--ll-accent)]" />
                        </div>
                        <div className="text-xs font-medium uppercase tracking-[0.18em] text-[var(--ll-muted)]">semantic map</div>
                    </div>

                    <div className="grid gap-0 lg:grid-cols-[1fr_15rem]">
                        <div className="p-5 sm:p-6">
                            <CardItem translateZ={76} className="flex w-full items-center gap-3 border border-[color:var(--ll-border)] bg-[var(--ll-surface-solid)] px-4 py-3 shadow-sm">
                                <Search className="h-4 w-4 text-[var(--ll-accent)]" />
                                <span className="text-sm text-[var(--ll-soft)]">find the auth article with the redirect bug</span>
                            </CardItem>
                                    ...
                                    >
                                        <div className="flex items-start justify-between gap-4">
                                            <div>
                                                ...
                                                <p className="mt-1 text-xs text-[var(--ll-muted)]">{item.url}</p>
                                            </div>
                        ...
                        <div className="border-t border-[color:var(--ll-border)] bg-[var(--ll-deep)] p-5 text-[var(--ll-deep-text)] lg:border-l lg:border-t-0">
                            <div className="text-xs font-medium uppercase tracking-[0.18em] text-[var(--ll-deep-muted)]">extension</div>
                            <CardItem translateZ={88} className="mt-5 flex items-center gap-3">
                                <Image src="/logo-transparent.png" alt="" width={42} height={42} className="rounded-md bg-[var(--ll-bg)] p-1" />
                                <div>
                                    <p className="text-sm font-semibold">Ready to organize</p>
                                    <p className="text-xs text-[var(--ll-deep-muted)]">Chrome bookmarks</p>
                                </div>
                            </CardItem>
                            <CardItem as="button" translateZ={72} className="mt-6 w-full bg-[var(--ll-primary)] px-4 py-3 text-sm font-semibold text-white">
                                Organize Bookmarks
                            </CardItem>
                            <div className="mt-4 space-y-2 text-xs text-[var(--ll-deep-muted)]">
                                <p className="flex items-center gap-2">
                                    <CircleCheck className="h-3.5 w-3.5 text-[var(--ll-accent)]" /> import structure
                                </p>
                                <p className="flex items-center gap-2">
                                    <CircleCheck className="h-3.5 w-3.5 text-[var(--ll-accent)]" /> cluster by topic
                                </p>
                                <p className="flex items-center gap-2">
                                    <CircleCheck className="h-3.5 w-3.5 text-[var(--ll-accent)]" /> review before apply
                                </p>
                            </div>
                        </div>
                    </div>
    ...
    return (
        <div className="ll-home min-h-screen bg-[var(--ll-bg)] text-[var(--ll-text)]">
            <header className="sticky top-0 z-50 border-b border-[color:var(--ll-border)] bg-[color-mix(in_oklab,var(--ll-bg)_90%,transparent)] backdrop-blur-xl">
                <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-5 sm:px-6 lg:px-8">
                    ...
                    <nav className="hidden items-center gap-7 text-sm font-medium text-[var(--ll-muted)] md:flex">
                        <Link href="#workflow" className="hover:text-[var(--ll-text)]">Workflow</Link>
                        <Link href="#pricing" className="hover:text-[var(--ll-text)]">Pricing</Link>
                    </nav>
                    <Link
                        href="/login"
            ...
            <main>
                    ...
                    <div className="mx-auto grid max-w-7xl items-center gap-12 px-5 py-14 sm:px-6 sm:py-20 lg:grid-cols-[0.9fr_1.1fr] lg:px-8 lg:py-24">
                        <div>
                            ...
                            <p className="mt-6 max-w-xl text-xl leading-8 text-[var(--ll-soft)]">
                                Turn years of saved tabs into a searchable map of what you meant to remember.
                            </p>
                ...
                <section id="workflow" className="border-b border-[color:var(--ll-border)] bg-[var(--ll-bg-soft)] py-18 sm:py-24">
                    <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
                        <div className="grid gap-10 lg:grid-cols-[0.8fr_1.2fr]">
                            <div>
                                <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--ll-accent)]">How it works</p>
                            ...
                            <div className="grid gap-4 sm:grid-cols-3">
                                {workflow.map((item, index) => (
                                    <div key={item} className="border-l border-[color:var(--ll-border-strong)] pl-5">
                                        <p className="text-sm font-semibold text-[var(--ll-primary)]">0{index + 1}</p>
                                        <p className="mt-4 text-base leading-7 text-[var(--ll-soft)]">{item}</p>
                                    </div>
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="mt-16 grid gap-5 lg:grid-cols-3">
                            {capabilities.map((item) => {
                                ...
                                return (
                                    <article key={item.label} className="border border-[color:var(--ll-border)] bg-[var(--ll-bg)] p-6">
                                        <Icon className="h-6 w-6 text-[var(--ll-accent)]" />
                                        <h3 className="mt-8 text-xl font-semibold tracking-tight">{item.label}</h3>
                                        <p className="mt-3 text-sm leading-6 text-[var(--ll-muted)]">{item.copy}</p>
                                    </article>
                                )
                            })}
                        </div>
                ...
                <section className="bg-[var(--ll-deep)] py-18 text-[var(--ll-deep-text)] sm:py-24">
                    <div className="mx-auto grid max-w-7xl gap-10 px-5 sm:px-6 lg:grid-cols-[1fr_1fr] lg:px-8">
                        <div>
                            <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--ll-deep-muted)]">Built for saved context</p>
                            <h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-5xl">
                        ...
                        <div className="space-y-6 text-lg leading-8 text-[var(--ll-deep-soft)]">
                            <p>
                                They are traces of projects, rabbit holes, research, fixes, purchases, and ideas. Link Loom treats them like connected material, not a pile of URLs.
                            </p>
                            <p>
                                The dashboard is for retrieval. The extension is for capture and review. The AI handles first-pass structure so you can spend attention where it matters.
                            </p>
                        </div>
                    </div>
                </section>

                <section id="pricing" className="border-b border-[color:var(--ll-border)] bg-[var(--ll-bg)] py-18 sm:py-24">
                    <div className="mx-auto max-w-7xl px-5 sm:px-6 lg:px-8">
                        <div className="grid gap-10 lg:grid-cols-[0.85fr_1.15fr]">
                            <div>
                                <p className="text-sm font-semibold uppercase tracking-[0.18em] text-[var(--ll-accent)]">Pricing</p>
                                <h2 className="mt-4 text-3xl font-semibold tracking-tight sm:text-5xl">
                                    Free to start. One payment for Pro.
                                </h2>
                                <p className="mt-5 max-w-lg text-base leading-7 text-[var(--ll-muted)]">
                                    Start with up to 500 bookmarks. Upgrade when Link Loom becomes part of your daily retrieval system.
                                </p>
                            </div>
                            ...
                            <div className="grid gap-5 md:grid-cols-[1fr_0.8fr]">
                                <div className="border border-[color:var(--ll-border)] bg-[var(--ll-surface)] p-6">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <h3 className="text-2xl font-semibold tracking-tight">Pro Membership</h3>
                                    </div>
                                ...
                                <div className="bg-[var(--ll-deep)] p-6 text-white">
                                    <p className="text-sm font-medium text-[var(--ll-deep-muted)]">Lifetime</p>
                                    <p className="mt-5 flex items-end gap-2">
                                        <span className="text-6xl font-semibold tracking-tight">$29</span>
                                        <span className="pb-2 text-sm text-[var(--ll-deep-muted)]">one time</span>
                                    </p>
                                    <Link
                                        href="/login"
            ...
            <footer className="border-t border-[color:var(--ll-border)] bg-[var(--ll-deep)] px-5 py-10 text-[var(--ll-deep-text)] sm:px-6 lg:px-8">
                <div className="mx-auto flex max-w-7xl flex-col gap-6 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                        <LogoMark inverse />
                        <p className="mt-3 text-sm text-[var(--ll-deep-muted)]">Copyright 2026 Link Loom. All rights reserved.</p>
                    </div>
                    <div className="flex flex-col gap-4 sm:items-end">
                        <div className="flex flex-wrap gap-4 text-sm text-[var(--ll-deep-soft)]">
                            <a href="mailto:support@linkloom.org" className="hover:text-white">support@linkloom.org</a>
                        ...
                        <div className="inline-flex items-center gap-2 bg-[var(--ll-bg)] px-4 py-2 text-sm text-[var(--ll-muted)]">
                            <span>Made with</span>
                            <span className="text-[var(--ll-primary)]" aria-hidden="true">heart</span>
                            <span>by</span>
                            <Image src="/crest-logo.png" alt="Crest Code Logo" width={20} height={20} className="h-5 w-5 object-contain" />
                            <a
