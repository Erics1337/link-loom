...
if (process.env.NODE_ENV !== 'test') {
  const cleanupTimer = setInterval(pruneExpiredBuckets, BUCKET_CLEANUP_INTERVAL_MS)
...
export const rateLimit = async ({ key, limit, windowMs }: RateLimitOptions) => {
  ...
  try {
    ...
    if (!result?.allowed) {
      return NextResponse.json(
        { error: 'Too many requests. Please try again later.' },
        {
          status: 429,
          headers: {
            'Retry-After': String(result?.retry_after_seconds ?? Math.ceil(windowMs / 1000)),
          },
        }
      )
    }
    ...
    console.error('Shared rate limit check failed; falling back to in-memory limiter.', error)
    return inMemoryRateLimit({ key, limit, windowMs })
  }
}

export const sanitizeApiError = (logPrefix: string, error: unknown, message = 'Request failed') => {
