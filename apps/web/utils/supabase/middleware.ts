...
export async function updateSession(request: NextRequest) {
  ...
  if (user && isProtectedPath && INACTIVITY_TIMEOUT_MS > 0) {
    const now = Date.now()
    const lastSeen = Number.parseInt(request.cookies.get(LAST_SEEN_COOKIE)?.value ?? '0', 10)

    if (lastSeen && now - lastSeen > INACTIVITY_TIMEOUT_MS) {
      await supabase.auth.signOut()

      if (request.nextUrl.pathname.startsWith('/api/')) {
        const expiredResponse = NextResponse.json({ error: 'Session expired' }, { status: 401 })
        expiredResponse.cookies.set(LAST_SEEN_COOKIE, '', { path: '/', maxAge: 0 })
        return expiredResponse
      }

      const redirectResponse = NextResponse.redirect(new URL('/login?message=session_expired', request.nextUrl))
      redirectResponse.cookies.set(LAST_SEEN_COOKIE, '', { path: '/', maxAge: 0 })
