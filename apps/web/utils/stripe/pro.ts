...
export const startProCheckoutForUser = async ({
  ...
  if (upsertError) {
    throw new Error(
      `Failed to initialize billing profile: ${upsertError.message}`,
    );
  }
...
export const applyCheckoutSessionToUser = async (
  ...
  if (session.mode === "payment") {
    ...
    if (error) {
      throw new Error(
        `Failed to activate lifetime Pro access: ${error.message}`,
      );
    }
    return;
  }
  ...
  if (session.mode === "subscription" && session.subscription) {
    ...
    if (error) {
      throw new Error(
        `Failed to activate subscription Pro access: ${error.message}`,
      );
    }
  }
};
